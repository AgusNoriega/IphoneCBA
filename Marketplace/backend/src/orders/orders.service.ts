...
export class OrdersService {
  ...
  private async currentOrder(username: string) {
  ...
  async addItem(productId: string, qty: number = 1) {
    return this.prisma.$transaction(async (tx) => {
      ...
      if (product.disponibilidad.Nombre === 'Vendido') {
        throw new BadRequestException('Producto ya vendido');
      }
      ...
      if (existingItem) {
        throw new BadRequestException('Producto ya agregado al pedido');
      }
